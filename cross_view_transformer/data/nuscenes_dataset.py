...
CLASSES = STATIC + DIVIDER + DYNAMIC   # 类，包括数据的集合
    ...
    **dataset_kwargs
    ...
    for scene_name, scene_record in helper.get_scenes():
        ...
        data = NuScenesDataset(scene_name, scene_record, helper,
                               transform=transform, **dataset_kwargs)     # 场景名称，，
...
class NuScenesDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.nusc_map = helper.get_map(scene_record['log_token'])  #日志标识

        self.view = get_view_matrix(**bev)  #view,通过计算的出
    ...
    def parse_scene(self, scene_record, camera_rigs):
        data = []
        sample_token = scene_record['first_sample_token']  # 标签 第一个sample的token，从sample.json中可以索引出唯一sample

        while sample_token:
            sample_record = self.nusc.get('sample', sample_token)  # sample_token：从sample.json中索引出唯一对应的sample；
    ...
    def parse_sample_record(self, sample_record, camera_rig):
        ...
        egolidar = self.nusc.get('ego_pose', lidar_record['ego_pose_token'])  # ego_pose_token表示车辆姿态标识符，与ego_pose.json的token一致
        ...
        for cam_idx in camera_rig:
            ...
            egocam = self.nusc.get('ego_pose', cam_record['ego_pose_token'])  #  ego_pose_token：对应的ego车辆的token，可以从ego_pose中索引出来
            cam = self.nusc.get('calibrated_sensor', cam_record['calibrated_sensor_token'])  #calibrated_sensor_token：可以从calibrated_sensor.json中索引出对应的相机外参
                                                                    # 和内参，3和4就对应索引出上文所说的ego和camera的外参，sample的外参并不在这个表里，
                                                                           #  而是在sample_annotation.json中

            cam_from_egocam = self.parse_pose(cam, inv=True)
            egocam_from_world = self.parse_pose(egocam, inv=True)

            E = cam_from_egocam @ egocam_from_world @ world_from_egolidarflat  # 外参，好像包括很多参数  @ 表示矩阵的乘法
            I = cam['camera_intrinsic']    # 相机内参

            full_path = Path(self.nusc.get_sample_data_path(cam_token))
            image_path = str(full_path.relative_to(self.nusc.dataroot))

            cam_channels.append(cam_channel)   # 相机的通道
            intrinsics.append(I)               # 相机内参
        ...
        return {
            ...
            'cam_channels': cam_channels,    # ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT',   'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT']
